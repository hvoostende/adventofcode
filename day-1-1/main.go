...
func main() {
	...
	for _, v := range instructions {
		...
		fmt.Println(newCompas)
		fmt.Println("x:", x)
		fmt.Println("y:", y)
	}

	fmt.Println("The shortest path to the destination is: ", x+y)
}
...
the instructions on the Easter Bunny Recruiting Document the Elves intercepted
...
either turn left (L) or right (R) 90 degrees, then walk forward the given number
